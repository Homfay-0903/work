                        ...
                        <div v-for="(item, index) in infoList" :key="index">
                            ...
                            <div v-if="item.index === 0" class="box-pre">
                                ...
                                <van-radio-group v-model="item.radio" direction="horizontal">
                                    <van-radio
                                        icon-size="12px"
                                        @click="defaultDate(item.radio)"
                                        :name="ide"
                                        v-for="(itemm, ide) in item.select"
                                        :key="ide"
                                        >{{ itemm.name }}</van-radio
                                    >
                                </van-radio-group>
                            </div>
                            <!-- 根据条件 -->
                            <div v-else-if="dynamicSelect(item)" class="box-pre">
                                ...
                                <input
                                    ...
                                    type="number"
                                    v-model="item.week"
                                    @input="validator(item)"
                                    v-on:input="onInputHandler(item)"
                                />
                                <span
                                    v-if="item.week && item.index === 1 && infoList[0].radio === 1"
                                    class="box-pre-span"
                                    >周</span
                                >
                            ...
                            <div v-else-if="item.index === 9 || item.index === 10 || item.index === 11" class="box-pre">
                                ...
                                <van-checkbox-group v-model="item.result">
                                    <van-checkbox
                                        @click="validatorResult(item.index)"
                                        icon-size="12px"
                                        :name="ide"
                                        v-for="(itemm, ide) in item.select"
                                        :key="ide"
                                        >{{ itemm.name }}</van-checkbox
                                    >
                                </van-checkbox-group>
                            </div>
                        </div>
...
export default {
    ...
    computed: {
        ...
        rules() {
            switch (this.infoListArray[0].radio) {
                ...
                case 2:
                    return this.commonCompute(
                        [this.infoListArray[2], this.infoListArray[3]],
                        [this.infoListArray[4], this.infoListArray[5]]
                    )
                default:
                    return false
            }
    ...
    methods: {
        ...
        commonCompute(radioArr = [], weekArr = []) {
            return (
                [this.infoListArray[6], this.infoListArray[7], this.infoListArray[8]]
                    .concat(radioArr)
                    .every((item) => item.radio > -1) &&
                [this.infoListArray[9], this.infoListArray[10], this.infoListArray[11]].every(
                    (item) => item.result.length
                ) &&
        ...
        dynamicSelect(item) {
            return (
                (item.index === 1 && this.infoList[0].radio === 1) ||
                (item.index === 4 && (this.infoList[0].radio === 1 || this.infoList[0].radio === 2)) ||
                (item.index === 5 && this.infoList[0].radio === 2)
            )
        },
        //  单选条件应用
        radio(item) {
            return (
                (item.index === 2 && this.infoList[0].radio === 2) ||
                item.index === 6 ||
                item.index === 7 ||
                item.index === 8
            )
        ...
        submit() {
            ...
            const wmId = Number(this.$route.query.wmId)
            this.$apollo
                .query({
                    query: submitInfo,
                    variables: {
                        ...
                        exercisePatterns:
                            this.infoList[9].result.length > 0
                                ? JSON.stringify(
                                    this.infoList[9].result.map((item) => {
                                        return item + 1
                                    })
                                )
                                : '[]',
                        gomutiMode:
                            this.infoList[10].result.length > 0
                                ? JSON.stringify(
                                    this.infoList[10].result.map((item) => {
                                        return item + 1
                                    })
                                )
                                : '[]',
                        otherQuestion:
                            this.infoList[11].result.length > 0
                                ? JSON.stringify(
                                    this.infoList[11].result.map((item) => {
                                        return item + 1
                                    })
                                )
                                : '[]',
