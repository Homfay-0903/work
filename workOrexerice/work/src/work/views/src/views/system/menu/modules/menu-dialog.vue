<template>
    <ElDialog :title="dialogTitle" :model-value="visible" @update:model-value="handleCancel" width="860px" align-center
        ...
        <ArtForm ref="formRef" v-model="form" :items="formItems" :rules="rules" :span="width > 640 ? 12 : 24"
            :gutter="20" label-width="100px" :show-reset="false" :show-submit="false">
            <template #menuType>
...
import { fetchGetPermissionCodes } from '@/api/system-manage'
...
const permissionCodes = ref<Array<{ value: string; label: string }>>([])
...
const fetchPermissionCodes = async () => {
    try {
        const codes = await fetchGetPermissionCodes()
        console.log('获取到的权限码:', codes)
        // 将字符串数组转换为 { value: code, label: code } 的对象数组格式
        permissionCodes.value = (codes || []).map(code => ({
            value: code,
            label: code,
        }))
    } catch (error) {
...
const rules = reactive<FormRules>({
    ...
    authRoute: [{ required: true, type: 'array', min: 1, message: '请至少选择一个权限标识', trigger: 'change' }],
})
    ...
    if (form.menuType === 'menu') {
        ...
        return [
            ...
            },
            {
                ...
                key: 'authLabel',
                type: 'input',
                props: { placeholder: '如：add、edit' },
            },
            {
                label: '路由标识',
                key: 'authRoute',
                type: 'select',
                props: { placeholder: '请选择路由标识', options: permissionCodes.value, multiple: true },
            },
        ]
    }
})
...
const loadFormData = (): void => {
    ...
    if (form.menuType === 'menu') {
        ...
        form.authRoute = row.permissionCodes || []
