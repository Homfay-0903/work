...
<script>
import { bmwaistContrasInfo, bmBdaPointInfos, bmScanFileInfos, bmGirthAnalysis } from '@/assets/js/apolloGql.js'
...
export default {
    ...
    methods: {
        ...
        async fetchMidWaistAnalysis(scanId) {
            ...
            try {
                ...
                if (!resp || resp.code !== 200 || !resp.data || !resp.data.midWaistAnalysis) return null
                return this.safeJsonParse(resp.data.midWaistAnalysis)
            } catch (e) {
                console.log(e)
                return null
            }
        },
        async applyWaistPercentilesByAnalysis(latestScanId, contrastScanId) {
            const [latest, contrast] = await Promise.all([
                this.fetchMidWaistAnalysis(latestScanId),
                this.fetchMidWaistAnalysis(contrastScanId),
            ])
            if (latest && latest.over_ratio !== null && latest.over_ratio !== '') {
                this.waistPercentile = Number(latest.over_ratio) || 0
            }
            if (contrast && contrast.over_ratio !== null && contrast.over_ratio !== '') {
                this.lastWaistPercentile = Number(contrast.over_ratio) || 0
            } else if (!contrastScanId) {
            ...
            await this.$apollo
                ...
                .then((res) => {
                    ...
                    if (info.code === 200 && data2.latestBmWaist !== null) {
                        ...
                        // 用 bm_girth_analysis.mid_waist_analysis 的 over_ratio 覆盖“超越人群比例”
                        this.applyWaistPercentilesByAnalysis(
                            data2.latestBmWaist && data2.latestBmWaist.scanId ? data2.latestBmWaist.scanId : this.reportInfo.scanId,
                            data2.contrastBmWaist && data2.contrastBmWaist.scanId ? data2.contrastBmWaist.scanId : null
                        )
