 ...
 * @LastEditTime: 2025-06-21 14:46:39
 * @LastEditors: liutq
 -->
<template>
...
export default {
    ...
    computed: {
        ...mapGetters(['standPeople', 'deviceErrors', 'measureInfo', 'deviceUsable', 'deviceInfo'])
    },
    watch: {
        // 监听站人状态
        standPeople: {
    ...
    created() {
        window.logger.info('进入体成分引导页')
        this.$store.commit('CHANGE_SCAN_STATE', true)
        // 播放金属电极片引导语音
        this.$emit('on-audio-event', 'playAudio', 'standPeopleGuideForMass')
        // 检测体重硬件异常
        this.goToStartError()
        // 启动脚印闪烁
        this.startTime()
        this.$store.commit('CHANGE_MEASURE_INFO', {
            mass: { isMeasured: true }
        })
    ...
    methods: {
        ...
        audioEndCallback(file) {
            if (file === 'standPeopleGuideForMass') {
                this.readyJump()
            }
        },
        // 开始倒计时
