<template>
    <div class="iframe-template" :style="options.spine ? { height: isDeviceReport ? '740px' : '260px' } : {}">
        <div class="iframe-template">
            ...
            <template v-else>
                <iframe v-if="!options.failed" :id="frameId" :name="frameId" frameborder="0" scrolling="no" ref="modelView"
                    class="iframe1" :src="`../../../../static/model-iframe/${options.url}?version=${version}`"
...
export default {
    ...
    //             console.log(`传入数据变化${JSON.stringify(val.errorInfo)} ${JSON.stringify(oldVal.errorInfo)}  ${JSON.stringify(this.options.errorInfo)}`)
    //             this.MODEL_STORE.options = this.options
    //             // document.getElementById(this.iframeId).contentWindow.location.reload()
    //         }
    //     }
    // },
    created: function () {
        this.MODEL_STORE.options = this.options
        console.log(`传入的数据${JSON.stringify(this.options.errorInfo)}`)
        /* iframe加载完成回调 */
        this.MODEL_STORE.onIframeLoad = (uuid) => {
            this.uuid = uuid
            console.log('onIframeLoad:', uuid)
            this.$emit('isIframeLoad', true)
        }

        /* 模型加载完成回调 */
        this.MODEL_STORE.onModelLoad = (uuid) => {
            console.log('onModelLoad:', uuid)
            this.$emit('isModelLoad', true)
        }

        /* 模型加载失败回调 */
        this.MODEL_STORE.onModelLoadError = (type, isNew, uuid) => {
            console.log('onModelLoadError:', uuid)
            if (uuid === this.uuid) {
