    ...
    <div class="qt-feedback">
        ...
        <div class="qt-feedback-box">
            <div class="text-area">
                <textarea
                    name
                    id="qt-feedback-textarea"
                    v-model="introduct"
                    placeholder="请在此处输入你的问题或建议"
                    maxlength="140"
                ></textarea>
                <span class="letter-monitor">{{`${textareaVal} / 140`}}</span>
            </div>
            <div class="picture">
                <form class="picture">
                    <ul class="picture-box clearfix">
                        <v-touch
                            class="fl upload_img"
                            v-for=" (img, idx) in pictures"
                            :key="idx"
                            @tap="prePicture(img,idx)"
                        >
        ...
        >
            <div v-if="preview.src" class="picture-preview">
                <v-touch class="content" @tap="closePre">
                    <img :src="preview.src" />
                </v-touch>
                <div class="preview-tool">
                    <v-touch @tap="delPicture" class="del-btn">
        ...
        <mt-popup
            ...
            class="popup-message-tip-box popup-message-feedback-box"
...
import { upyunUrl } from '@/assets/js/config.js'
export default {
    data() {
        return {
            introduct: '',
    ...
    methods: {
        ...
        delPicture() {
            this.pictireUrls.splice(this.preview.idx, 1)
        ...
        handleUpload(event) {
            ...
            for (let i = 0; i < event.target.files.length; i++) {
                ...
                fileReader.onload = e => {
                    ...
                    this.Pretreatment(file, spark.end())
                }
                fileReader.onerror = () => {
            ...
            this.$apollo
                ...
                .then(res => {
                    ...
                    if (data && data.code === 200) {
                        ...
                        axios.post(data.data.url, params, config).then(response => {
                            if (response.data.code === 200) {
                                // 保存图片上传成功后返回url
                                this.pictireUrls.push(`${upyunUrl}${response.data.url}`)
                            } else {
                                this.$toast({
                                    message: '操作失败，请稍后再试!',
                                    duration: 3000
