...
<script>
import { memberLogin, findScanTypeInfo, findScanTrueHistory, getWechatSignature, ifLogoCloudEnable, getPillowReportQaByScanId } from '@/assets/js/apolloGql.js'
...
export default {
    ...
    created() {
        ...
        } else if (this.$route.query.no_jump) {
    ...
    methods: {
        ...
        jumpToReport(viewStatus) {
            this.shareClick()
            console.log('jumpToReport viewStatus:' + viewStatus)
            this.$router.push({
        ...
        async handleEnterMsg(newUser) {
            ...
            } else if (['N3'].includes(this.$route.query.device_id.slice(0, 2))) {
                window.location.href = `${homepageUrl}/#/transfer?type=Visbody-A%20Pro6&scanTime=${scanTime}&redirect_uri=${encodeURIComponent(window.location.href + (newUser ? '&new_user=1' : ''))}`
