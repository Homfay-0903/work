<template>
    <ElDialog v-model="visible" title="菜单权限" width="520px" align-center class="el-dialog-border" @close="handleClose">
        <ElScrollbar height="70vh">
            <ElTree ref="treeRef" :data="processedMenuList" show-checkbox node-key="name"
                :default-expand-all="isExpandAll" :default-checked-keys="[1, 2, 3]" :props="defaultProps"
...
const setCheckedPermissions = () => {
    ...
    // 从角色数据中提取所有 actionId
    const roleActionIds = props.roleData.roleMenus.map(rm => rm.actionId)

    // 递归遍历处理后的菜单树，找到所有匹配的权限节点的 name
    const checkedKeys: string[] = []
    const findMatchingNodes = (nodes: MenuNode[]) => {
        nodes.forEach(node => {
            // 如果是权限节点（isAuth 为 true），且 actionId 在角色权限列表中
            if (node.isAuth && node.actionId && roleActionIds.includes(node.actionId)) {
